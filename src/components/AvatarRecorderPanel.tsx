...
export const AvatarRecorderPanel: React.FC<AvatarRecorderPanelProps> = ({ sourceRef, fileBaseName }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`grid grid-cols-3 gap-2 text-center font-mono text-[9px] ${
      ...
      >
        ...
        >
          ...
          <strong
            className={`block mt-1 ${recorder.isRecording || recorder.isGifEncoding ? 'text-rose-500 dark:text-rose-300' : 'text-emerald-600 dark:text-emerald-400'}`}
          >
            {recorder.isRecording
              ? copy.recording
              : recorder.isGifEncoding
                ? `${copy.gifEncoding} ${formatPercent(recorder.gifProgress)}`
                : recorder.clip || recorder.gifClip
                  ? copy.ready
                  : copy.idle}
          </strong>
        </div>
      ...
      {!recorder.isSupported && (
        <p
          className="rounded-sm border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-[10px] text-amber-700 dark:text-amber-300"
          role="status"
        >
          {copy.unsupported}
        </p>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          ...
          <span>
            {recorder.isGifEncoding ? `${copy.gifEncoding} ${formatPercent(recorder.gifProgress)}` : copy.gif}
          </span>
        </button>
      ...
      {recorder.clip && (
        <div className="flex items-center justify-between gap-3 text-[10px] text-slate-500 dark:text-white/50">
          <span>
            {formatBytes(recorder.clip.sizeBytes)} · {formatRecordingDuration(recorder.clip.durationMs)}
          </span>
          <button
            type="button"
      ...
      {recorder.gifClip && (
        <div className="flex items-center justify-between gap-3 text-[10px] text-slate-500 dark:text-white/50">
          <span>
            GIF · {formatBytes(recorder.gifClip.sizeBytes)} · {formatRecordingDuration(2000)}
          </span>
          <button
            type="button"
      ...
      <p className="text-[9px] text-slate-500 dark:text-white/45 font-mono leading-relaxed">{copy.gifNote}</p>
    </section>
  );
};
