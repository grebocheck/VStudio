import { AvatarConfig, RigParams, PresetAvatar } from './types';

/** Neutral rig calibration — every tracking/animation frame starts from here. */
export const INITIAL_RIG: RigParams = {
  eyeLOpen: 1.0,
  eyeROpen: 1.0,
  mouthOpen: 0.0,
  mouthForm: 0.5,
  eyebrowY: 0,
  pupilX: 0,
  pupilY: 0,
  angleX: 0,
  angleY: 0,
  angleZ: 0,
  breath: 0,
  bodyX: 0,
  tongueOut: 0.0,
};

/**
 * Built-in starter characters. Each `id` is a stable localization key — the
 * matching `<id>_name` / `<id>_lore` entries live in src/i18n/{en,uk}.ts.
 * The `name`/`lore` baked into `config` are the authored fallback used when no
 * translation exists (e.g. for custom or AI-generated avatars).
 */
export const PRESETS: PresetAvatar[] = [
  {
    id: 'cyber-neko',
    name: 'Мія (Cyber Neko)',
    config: {
      skinColor: '#f5f3ff',
      eyeColor: '#06b6d4',
      pupilStyle: 'slit',
      pupilColor: '#f43f5e',
      eyebrowStyle: 'normal',
      eyebrowColor: '#4f46e5',
      hairStyleBang: 'wolf-cut',
      hairStyleBack: 'drill-tails',
      hairColor: '#4f46e5',
      hairHighlightColor: '#a855f7',
      clothingStyle: 'cyber-ninja',
      clothingColor1: '#111827',
      clothingColor2: '#22d3ee',
      accessoryStyle: 'neko-ears',
      accessoryColor: '#f43f5e',
      backgroundStyle: 'gaming',
      name: 'Мія',
      lore: 'Кібернетична дівчинка-кішка, яка веде стріми з віртуального неонового Токіо. Хоч вона інколи буває забудькувата, її життєрадісність та драйв притягують тисячі глядачів!',
      blushOpacity: 0.25,
      blushColor: '#ff4d6d',
      hasFangs: false,
      earStyle: 'normal',
      hairGradient: 'sunset',
      accessoryGlow: true,
      headSize: 1.05,
      neckWidth: 0.82,
      neckHeight: 0.9,
      shoulderWidth: 0.94,
      clothingPrint: 'cyber',
      activeEmotion: 'smug',
      artStyle: 'anime',
      faceShape: 'sharp',
      eyeShape: 'cat-eye',
    },
  },
  {
    id: 'vampire-princess',
    name: 'Міцукі (Goth Vampire)',
    config: {
      skinColor: '#fafaf9',
      eyeColor: '#f43f5e',
      pupilStyle: 'heart',
      pupilColor: '#ffe4e6',
      eyebrowStyle: 'thin',
      eyebrowColor: '#18181b',
      hairStyleBang: 'hime',
      hairStyleBack: 'hime-long',
      hairColor: '#18181b',
      hairHighlightColor: '#be123c',
      clothingStyle: 'lolita-dress',
      clothingColor1: '#18181b',
      clothingColor2: '#f43f5e',
      accessoryStyle: 'horns',
      accessoryColor: '#f43f5e',
      backgroundStyle: 'dark-studio',
      name: 'Міцукі',
      lore: 'Готична вампірська лоліта, яка замість сну в склепі до світанку затято розгадує головоломки та проходить хардкорні RPG. Обожнює кажанів, чай з журавлини та іклавий рум’янець.',
      blushOpacity: 0.45,
      blushColor: '#e11d48',
      hasFangs: true,
      earStyle: 'pointy',
      hairGradient: 'sunset',
      accessoryGlow: true,
      headSize: 1.08,
      neckWidth: 0.76,
      neckHeight: 0.88,
      shoulderWidth: 0.9,
      clothingPrint: 'cross',
      activeEmotion: 'angry',
      artStyle: 'anime',
      faceShape: 'sharp',
      eyeShape: 'sharp',
    },
  },
  {
    id: 'astral-elf',
    name: 'Луна (Astral Star Elf)',
    config: {
      skinColor: '#f0fdfa',
      eyeColor: '#8b5cf6',
      pupilStyle: 'star',
      pupilColor: '#fef08a',
      eyebrowStyle: 'normal',
      eyebrowColor: '#7c3aed',
      hairStyleBang: 'curtain-bangs',
      hairStyleBack: 'wavy',
      hairColor: '#6d28d9',
      hairHighlightColor: '#a78bfa',
      clothingStyle: 'sweater',
      clothingColor1: '#2e1065',
      clothingColor2: '#a78bfa',
      accessoryStyle: 'glasses',
      accessoryColor: '#fef08a',
      backgroundStyle: 'nebula',
      name: 'Луна',
      lore: 'Ельфійська хранителька бібліотеки зіркового світла. Вона обожнює слухати ASMR звуки нічного космосу, ховатися в теплий в’язаний светр і розповідати казки про далекі туманності.',
      blushOpacity: 0.2,
      blushColor: '#ec4899',
      hasFangs: false,
      earStyle: 'elf',
      hairGradient: 'indigo-fade',
      accessoryGlow: false,
      headSize: 1.0,
      neckWidth: 0.8,
      neckHeight: 1.0,
      shoulderWidth: 0.94,
      clothingPrint: 'heart',
      activeEmotion: 'none',
      artStyle: 'anime',
      faceShape: 'mature',
      eyeShape: 'droopy',
    },
  },
  {
    id: 'anime-idol',
    name: 'Кіра (Crimson Star)',
    config: {
      skinColor: '#fff5f5',
      eyeColor: '#06b6d4',
      pupilStyle: 'diamond',
      pupilColor: '#0f172a',
      eyebrowStyle: 'thin',
      eyebrowColor: '#e11d48',
      hairStyleBang: 'hime',
      hairStyleBack: 'bun',
      hairColor: '#e11d48',
      hairHighlightColor: '#fda4af',
      clothingStyle: 'royal-knight',
      clothingColor1: '#ffffff',
      clothingColor2: '#e11d48',
      accessoryStyle: 'neko-ears',
      accessoryColor: '#e11d48',
      backgroundStyle: 'nebula',
      name: 'Кіра',
      lore: 'Червоноволоса аніме-кішкодівчина в стилі High School DxD. Її шовковисте волосся кольору багряного серця прикрашає грайливий ахоге (пасмо), а сяючі аквамаринові очі та класична школа-самурайка підкорюють серця.',
      blushOpacity: 0.35,
      blushColor: '#f43f5e',
      hasFangs: false,
      earStyle: 'normal',
      hairGradient: 'sunset',
      accessoryGlow: false,
      headSize: 1.04,
      neckWidth: 0.72,
      neckHeight: 0.95,
      shoulderWidth: 0.9,
      clothingPrint: 'heart',
      activeEmotion: 'happy',
      artStyle: 'anime',
      faceShape: 'sharp',
      eyeShape: 'sharp',
    },
  },
  {
    id: 'koneko-dxd',
    name: 'Конеко (White Kitten)',
    config: {
      skinColor: '#fffaf8',
      eyeColor: '#f59e0b',
      pupilStyle: 'round',
      pupilColor: '#3c1e02',
      eyebrowStyle: 'thin',
      eyebrowColor: '#7c2d12',
      hairStyleBang: 'hime',
      hairStyleBack: 'short',
      hairColor: '#e5e7eb',
      hairHighlightColor: '#ffffff',
      clothingStyle: 'lolita-dress',
      clothingColor1: '#ffffff',
      clothingColor2: '#0f172a',
      accessoryStyle: 'neko-ears',
      accessoryColor: '#e5e7eb',
      backgroundStyle: 'dark-studio',
      name: 'Конеко',
      lore: 'Срібноволоса дівчина-кішка в стилі легендарної Конеко з High School DxD. Завжди спокійна й незворушна зовні, вона постає перед вами в чарівному готичному костюмі покоївки із шовковистими білими вушками та сяючими золотими очками, що підкорять ваше серце!',
      blushOpacity: 0.4,
      blushColor: '#ff2e55',
      hasFangs: false,
      earStyle: 'normal',
      hairGradient: 'none',
      accessoryGlow: false,
      headSize: 1.05,
      neckWidth: 0.74,
      neckHeight: 0.94,
      shoulderWidth: 0.92,
      clothingPrint: 'heart',
      activeEmotion: 'none',
      artStyle: 'anime',
      faceShape: 'chubby',
      eyeShape: 'almond',
    },
  },
  {
    id: 'retro-pip',
    name: 'Піп (1930s Rubber Hose)',
    config: {
      skinColor: '#f5efe0',
      eyeColor: '#111111',
      pupilStyle: 'round',
      pupilColor: '#000000',
      eyebrowStyle: 'thick',
      eyebrowColor: '#111111',
      hairStyleBang: 'short',
      hairStyleBack: 'short',
      hairColor: '#18181b',
      hairHighlightColor: '#3f3f46',
      clothingStyle: 'suit',
      clothingColor1: '#111111',
      clothingColor2: '#ffffff',
      accessoryStyle: 'none',
      accessoryColor: '#111111',
      backgroundStyle: 'dark-studio',
      name: 'Піп',
      lore: 'Класичний чорно-білий мультиплікаційний герой епохи 1930-х. Він постійно весело пружинить і коливається під уявні джазові синкопи, носить стильний круглий фрак та робить комічні очі!',
      blushOpacity: 0.15,
      blushColor: '#000000',
      hasFangs: false,
      earStyle: 'pointy',
      hairGradient: 'none',
      accessoryGlow: false,
      headSize: 1.1,
      neckWidth: 0.7,
      neckHeight: 0.85,
      shoulderWidth: 0.85,
      clothingPrint: 'cross',
      activeEmotion: 'shocked',
      artStyle: 'retro',
    },
  },
  {
    id: 'kitsune-oracle',
    name: 'Акарі (Kitsune Oracle)',
    config: {
      skinColor: '#fff7ed',
      eyeColor: '#f59e0b',
      pupilStyle: 'slit',
      pupilColor: '#7c2d12',
      eyebrowStyle: 'thin',
      eyebrowColor: '#9a3412',
      hairStyleBang: 'cross-bangs',
      hairStyleBack: 'wavy',
      hairColor: '#f97316',
      hairHighlightColor: '#fde68a',
      clothingStyle: 'kimono',
      clothingColor1: '#7f1d1d',
      clothingColor2: '#facc15',
      accessoryStyle: 'fox-mask',
      accessoryColor: '#facc15',
      backgroundStyle: 'gaming',
      name: 'Акарі',
      lore: 'Оракул у масці кіцуне, яка стрімить нічний фольклор, затишні стратегічні ігри та спокійні ворожіння з неонової храмової студії. Золоті акценти й лисича маска роблять кожну сцену церемонною.',
      blushOpacity: 0.28,
      blushColor: '#fb7185',
      hasFangs: true,
      earStyle: 'pointy',
      hairGradient: 'linear',
      accessoryGlow: true,
      headSize: 1.03,
      neckWidth: 0.78,
      neckHeight: 0.92,
      shoulderWidth: 0.92,
      clothingPrint: 'star',
      activeEmotion: 'smug',
      artStyle: 'anime',
      faceShape: 'mature',
      eyeShape: 'cat-eye',
    },
  },
  {
    id: 'sakura-idol',
    name: 'Сакура (Sakura Idol)',
    config: {
      skinColor: '#fff5f5',
      eyeColor: '#ec4899',
      pupilStyle: 'diamond',
      pupilColor: '#ffffff',
      eyebrowStyle: 'thin',
      eyebrowColor: '#f472b6',
      hairStyleBang: 'curtain-bangs',
      hairStyleBack: 'ponytail',
      hairColor: '#f472b6',
      hairHighlightColor: '#fda4af',
      clothingStyle: 'idol-stage',
      clothingColor1: '#fdf2f8',
      clothingColor2: '#ec4899',
      accessoryStyle: 'crown',
      accessoryColor: '#fbbf24',
      backgroundStyle: 'nebula',
      name: 'Сакура',
      lore: 'Зірка віртуальної сцени з діамантовими зіницями та рожевим хвостом. Вона мріє підкорити серця мільйонів своїм ніжним голосом та сяючою посмішкою. Кожен її стрім — це справжнє шоу!',
      blushOpacity: 0.4,
      blushColor: '#f472b6',
      hasFangs: false,
      earStyle: 'normal',
      hairGradient: 'sunset',
      accessoryGlow: true,
      headSize: 1.04,
      neckWidth: 0.76,
      neckHeight: 0.92,
      shoulderWidth: 0.92,
      clothingPrint: 'heart',
      activeEmotion: 'happy',
      artStyle: 'anime',
      faceShape: 'round',
      eyeShape: 'almond',
    },
  },
  {
    id: 'midnight-witch',
    name: 'Моргана (Midnight Witch)',
    config: {
      skinColor: '#f5f0ff',
      eyeColor: '#a855f7',
      pupilStyle: 'cross',
      pupilColor: '#c4b5fd',
      eyebrowStyle: 'thin',
      eyebrowColor: '#6d28d9',
      hairStyleBang: 'wolf-cut',
      hairStyleBack: 'wavy',
      hairColor: '#4c1d95',
      hairHighlightColor: '#a78bfa',
      clothingStyle: 'witch-robe',
      clothingColor1: '#1e1b4b',
      clothingColor2: '#a78bfa',
      accessoryStyle: 'witch-hat',
      accessoryColor: '#4c1d95',
      backgroundStyle: 'dark-studio',
      name: 'Моргана',
      lore: 'Таємнича відьма опівночі, яка стрімить зіркові карти таро, атмосферні RPG та нічну музику. Її хрестоподібні зіниці гіпнотизують, а містичний плащ мерехтить зоряними візерунками.',
      blushOpacity: 0.2,
      blushColor: '#c084fc',
      hasFangs: false,
      earStyle: 'pointy',
      hairGradient: 'indigo-fade',
      accessoryGlow: true,
      headSize: 1.02,
      neckWidth: 0.78,
      neckHeight: 0.95,
      shoulderWidth: 0.94,
      clothingPrint: 'star',
      activeEmotion: 'none',
      artStyle: 'anime',
      faceShape: 'mature',
      eyeShape: 'sharp',
    },
  },
  {
    id: 'dragon-knight',
    name: 'Драгомира (Dragon Knight)',
    config: {
      skinColor: '#fdf2e9',
      eyeColor: '#f59e0b',
      pupilStyle: 'cat-vertical',
      pupilColor: '#7c2d12',
      eyebrowStyle: 'thick',
      eyebrowColor: '#7f1d1d',
      hairStyleBang: 'asymmetric',
      hairStyleBack: 'layered',
      hairColor: '#b91c1c',
      hairHighlightColor: '#fbbf24',
      clothingStyle: 'royal-knight',
      clothingColor1: '#7f1d1d',
      clothingColor2: '#fbbf24',
      accessoryStyle: 'horns',
      accessoryColor: '#b45309',
      backgroundStyle: 'dark-studio',
      name: 'Драгомира',
      lore: 'Лицарка драконячої крові, що захищає свій чат, як власне золото. Шрам через око — пам’ять про двобій із грифоном, а бурштинові очі з вертикальними зіницями бачать кожен токсичний коментар за милю.',
      blushOpacity: 0.15,
      blushColor: '#ef4444',
      hasFangs: false,
      earStyle: 'elf',
      hairGradient: 'sunset',
      accessoryGlow: true,
      headSize: 1.02,
      neckWidth: 0.85,
      neckHeight: 0.95,
      shoulderWidth: 1.05,
      clothingPrint: 'none',
      activeEmotion: 'none',
      artStyle: 'anime',
      faceShape: 'sharp',
      eyeShape: 'sharp',
      eyelashStyle: 'minimal',
      irisStyle: 'gemstone',
      eyeHighlightStyle: 'star-glint',
      toothStyle: 'sharp-teeth',
      mouthShape: 'wide',
      faceScar: 'eye-scar',
      earDecoration: 'cuff',
    },
  },
  {
    id: 'mermaid-princess',
    name: 'Марина (Mermaid Princess)',
    config: {
      skinColor: '#f0fdfa',
      eyeColor: '#0d9488',
      pupilStyle: 'round',
      pupilColor: '#134e4a',
      eyebrowStyle: 'thin',
      eyebrowColor: '#0f766e',
      hairStyleBang: 'curtain-bangs',
      hairStyleBack: 'fishtail-braid',
      hairColor: '#14b8a6',
      hairHighlightColor: '#99f6e4',
      clothingStyle: 'chinese-dress',
      clothingColor1: '#0f766e',
      clothingColor2: '#fbcfe8',
      accessoryStyle: 'flower-crown',
      accessoryColor: '#f9a8d4',
      backgroundStyle: 'nebula',
      name: 'Марина',
      lore: 'Принцеса підводного королівства, яка випливла на поверхню заради косплей-стрімів і морських легенд. Її коса сплетена за технікою «риб’ячий хвіст», а перламутрові губи розповідають історії затонулих кораблів.',
      blushOpacity: 0.35,
      blushColor: '#f472b6',
      hasFangs: false,
      earStyle: 'elf',
      hairGradient: 'linear',
      accessoryGlow: false,
      headSize: 1.05,
      neckWidth: 0.78,
      neckHeight: 0.92,
      shoulderWidth: 0.9,
      clothingPrint: 'none',
      activeEmotion: 'none',
      artStyle: 'anime',
      faceShape: 'round',
      eyeShape: 'almond',
      eyelashStyle: 'glamour',
      freckles: true,
      frecklesDensity: 0.4,
      frecklesColor: '#c4847c',
      irisStyle: 'organic',
      eyeHighlightStyle: 'double-spark',
      mouthShape: 'pouty',
      lipStyle: 'glossy',
      lipColor: '#f9739b',
      earDecoration: 'piercing',
    },
  },
  {
    id: 'steampunk-inventor',
    name: 'Грета (Steampunk Inventor)',
    config: {
      skinColor: '#fde8d7',
      eyeColor: '#92400e',
      pupilStyle: 'round',
      pupilColor: '#451a03',
      eyebrowStyle: 'normal',
      eyebrowColor: '#92400e',
      hairStyleBang: 'braided-bangs',
      hairStyleBack: 'messy-bun',
      hairColor: '#c2702d',
      hairHighlightColor: '#fcd34d',
      clothingStyle: 'pirate-coat',
      clothingColor1: '#78350f',
      clothingColor2: '#d97706',
      accessoryStyle: 'glasses',
      accessoryColor: '#b45309',
      backgroundStyle: 'dark-studio',
      name: 'Грета',
      lore: 'Винахідниця з мідними окулярами та вічно забрудненими у мастилі рукавами. Стрімить збирання шестеренчастих роботів і парових машин. Щербинка між зубами свистить, коли вона сміється над невдалими прототипами.',
      blushOpacity: 0.3,
      blushColor: '#fb923c',
      hasFangs: false,
      earStyle: 'normal',
      hairGradient: 'none',
      accessoryGlow: false,
      headSize: 1.0,
      neckWidth: 0.82,
      neckHeight: 0.95,
      shoulderWidth: 0.96,
      clothingPrint: 'none',
      activeEmotion: 'happy',
      artStyle: 'classic',
      faceShape: 'default',
      eyeShape: 'default',
      eyelashStyle: 'natural',
      freckles: true,
      frecklesDensity: 0.7,
      frecklesColor: '#a16207',
      beautyMark: 'left-cheek',
      irisStyle: 'organic',
      eyeHighlightStyle: 'standard',
      toothStyle: 'gap-tooth',
      mouthShape: 'small',
      earDecoration: 'piercing',
    },
  },
  {
    id: 'galaxy-dj',
    name: 'Нова (Galaxy DJ)',
    config: {
      skinColor: '#ede9fe',
      eyeColor: '#6366f1',
      pupilStyle: 'star',
      pupilColor: '#e0e7ff',
      eyebrowStyle: 'thin',
      eyebrowColor: '#4338ca',
      hairStyleBang: 'messy',
      hairStyleBack: 'twintail-long',
      hairColor: '#4f46e5',
      hairHighlightColor: '#22d3ee',
      clothingStyle: 'punk-jacket',
      clothingColor1: '#1e1b4b',
      clothingColor2: '#22d3ee',
      accessoryStyle: 'headphones',
      accessoryColor: '#22d3ee',
      backgroundStyle: 'nebula',
      name: 'Нова',
      lore: 'Космічна діджейка, що міксує треки на орбітальній станції. У її очах вирують справжні галактики, неонові смуги під очима пульсують у такт бітам, а градієнтні губи світяться в ультрафіолеті.',
      blushOpacity: 0.2,
      blushColor: '#a78bfa',
      hasFangs: false,
      earStyle: 'normal',
      hairGradient: 'indigo-fade',
      accessoryGlow: true,
      headSize: 1.04,
      neckWidth: 0.8,
      neckHeight: 0.9,
      shoulderWidth: 0.95,
      clothingPrint: 'cyber',
      activeEmotion: 'none',
      artStyle: 'anime',
      faceShape: 'sharp',
      eyeShape: 'cat-eye',
      eyelashStyle: 'natural',
      facePaint: 'under-eye-stripe',
      irisStyle: 'galaxy',
      eyeHighlightStyle: 'double-spark',
      mouthShape: 'wide',
      lipStyle: 'gradient',
      lipColor: '#c026d3',
      earDecoration: 'feather',
    },
  },
];

/** Default starting avatar. */
export const DEFAULT_CONFIG: AvatarConfig = PRESETS[0].config;

type PresetStats = { presetStats: Record<string, string> };

/**
 * Resolve a built-in preset's localized name + lore by its id.
 * Returns null when `key` is null/unknown (custom or AI-generated avatar),
 * letting callers fall back to the config's own name/lore.
 */
export function localizePreset(key: string | null, t: PresetStats): { name: string; lore: string } | null {
  if (!key) return null;
  const stats = t.presetStats;
  const name = stats[`${key}_name`];
  const lore = stats[`${key}_lore`];
  if (name && lore) return { name, lore };
  return null;
}
