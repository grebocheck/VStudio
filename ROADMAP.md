...
6. ✅ **Telegram sticker pack:** production-style native vector `.tgs` generator на базі модельки персонажа: Bezier-риг без фону, 9 емоційних loop-стікерів із окремими параметрами анімації, `512x512`, `60fps`, до 3 секунд, preflight-валідація Telegram-обмежень, ZIP із `.tgs` файлами та manifest для @Stickers.
...
- [x] Генератор стікерів на основі модельки персонажа: реалізовано Telegram-native vector `.tgs` pack із Bezier-кривими, прозорим фоном, 9 emotion-specific loop-анімаціями, preflight-валідацією розміру/формату та ZIP/manifest експортом для @Stickers.
